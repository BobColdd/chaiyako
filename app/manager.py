...
def register_farmer():
    ...
    code = farmer.generate_verification_code()
    db.session.commit()

    send_sms(phone, f"Welcome to {center.name}. Your farm number is {farm_number}. "
                     f"Use code {code} at the Kericho Tea Tracker login to set your password.")

    flash(f"Farmer registered. Farm number {farm_number} created — verification code sent by SMS.", "success")
...
@manager_bp.route("/farmers/resend-code/<int:farmer_id>", methods=["POST"])
@login_required
@manager_required
def resend_code(farmer_id):
    farmer = Farmer.query.get_or_404(farmer_id)
    code = farmer.generate_verification_code()
    farmer.must_set_password = True
    db.session.commit()
    send_sms(farmer.phone, f"Your Kericho Tea Tracker verification code is {code}.")
    flash(f"New verification code sent to {farmer.full_name}.", "success")
    return redirect(url_for("manager.farmers"))
...
def staff():
    ...
    if request.method == "POST":
        ...
        elif FactoryUser.query.filter_by(username=username).first():
