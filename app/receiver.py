...
def home():
    ...
    if request.method == "POST":
        ...
        if not chosen or grade not in QualityRecord.GRADE_SCORES:
            flash(
                "Choose at least one buying centre and a quality grade.",
                "error",
            )
            return redirect(url_for("receiver.home"))

        for centre_id in chosen:
            existing = QualityRecord.query.filter_by(
                buying_centre_id=centre_id,
                date=record_date,
            ).first()
            ...
            if existing:
                ...
                existing.recorded_by_id = current_user.employee_id
                ...
                record = QualityRecord(
                    ...
                    grade=grade,
                    notes=notes or None,
                    recorded_by_id=current_user.employee_id,
                )
        ...
        flash(
            f"Quality recorded for {len(chosen)} buying centre(s).",
            "success",
        )
    ...
    return render_template(
        ...
        recent_quality=QualityRecord.query.order_by(
            QualityRecord.recorded_at.desc()
        ).limit(20).all(),
        today=today,
    )
...
def trends():
    ...
    for c in centres:
        direction, pct = trend_by_centre.get(
            c.id,
            ("stagnant", 0.0),
        )
        ...
        rows.append(
            {
                "centre": c,
                "today_kilos": today_by_centre.get(c.id, 0.0),
                "direction": direction,
                "pct_change": pct,
            }
        )
    ...
    return render_template(
        ...
        centres=centres,
        trend_rows=rows,
        today_total=total_kilos(today, today),
    )
...
def trends_compare():
    try:
        return compare_periods(
            request.args.get("mode", "days"),
            request.args.get("a", ""),
            request.args.get("b", ""),
            request.args.get("centre_id", type=int) or None,
        )
    except (ValueError, IndexError, TypeError):
        return {
            "error": "Enter two valid dates to compare."
        }, 400
