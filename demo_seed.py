...
def _seed():
    ...
    for idx, full_name in enumerate(farmer_names, start=1):
        ...
        farmer = Farmer(full_name=full_name, phone=phone, scale=scale, must_set_password=False)
    ...
    # --- A light sprinkling of farmer-diary data (plucking, pruning, tools, notes) ---
    # for the first 8 farms, so the farmer-facing screens aren't empty in the demo.
    prune_types = ["skiffing", "medium pruning", "deep pruning", "tipping"]
    tool_names = ["Pruning shears", "Panga", "Plucking baskets", "Wheelbarrow", "Sprayer"]
    for farm in farms[:8]:
        for i in range(3):
            db.session.add(PluckingRecord(
                farm_id=farm.id, date=date.today() - timedelta(days=random.randint(1, 14)),
                kilos=round(random.uniform(8, 30), 1), pluckers_count=random.randint(1, 4),
            ))
        db.session.add(PruningRecord(
            farm_id=farm.id, date=date.today() - timedelta(days=random.randint(10, 90)),
            bushes_pruned=random.randint(200, 1500), prune_type=random.choice(prune_types),
        ))
        farmer = next(f for f in farmers if f.id == farm.farmer_id)
        db.session.add(Tool(farmer_id=farmer.id, name=random.choice(tool_names),
                             quantity=random.randint(1, 3), cost=round(random.uniform(300, 3500), 0)))
        db.session.add(Note(farmer_id=farmer.id, farm_id=farm.id, title="Reminder",
                             content="Check on the new tea seedlings near the fence line."))
    ...
    for full_name, username, pin in TALLYBOYS:
        print(f"  {full_name:<20} -> username: {username}   password: clerk123   pin: {pin}")
    print()
    print("=" * 60)
    print("FARMER LOGIN — visit /login  (password for all demo farmers: farmer123)")
    print("=" * 60)
    print(f"  {len(farmers)} demo farmers created, phones 0712345001 .. 0712345{len(farmers):03d}")
    print(f"  e.g. {farmers[0].full_name} -> phone: {farmers[0].phone}")
    print()
